...
# --- AVERIO / LAIVADATA (SCRAPING + AIKASUODATIN) ---
@st.cache_data(ttl=300)
def fetch_live_ships():
    # Tunnettujen raskaiden alusten fysiikka ja satamadata
    ship_database = {
    ...
    # 1. YRITETÄÄN KIRURGISTA RAAPIMISTA (AVERIO)
    try:
        ...
        if res.status_code == 200:
            ...
            for tr in soup.find_all('tr'):
                ...
                if len(tds) >= 4:
                    ...
                    if "Saapumisaika" in time_str or not ship_name:
                        continue
                    ...
                    if clean_name in ship_database:
                        ...
                        try:
                            ...
                            # Yli 12 tuntia menneisyydessä oleva laiva on todennäköisesti seuraavan vuorokauden puolella (esim. klo 00:30)
                            if (now - ship_dt).total_seconds() > 12 * 3600:
                                ship_dt += timedelta(days=1)
                                
                            # Hylätään säälittä laivat, joiden arvioitu saapuminen oli yli 45 minuuttia sitten
                            if ship_dt < now - timedelta(minutes=45):
                                continue
                        except Exception:
                            pass # Jos ajan muunnos epäonnistuu oudon formaatin takia, annetaan mennä läpi suodattimesta
                        # --------------------
                        ...
                        if pax_digits:
                            ...
                            if pax_count > 0:
                                ...
                                ships.append({
                                    ...
                                    "source": "LÄHDE: AVERIO.FI (LIVE RAAPINTA)"
                                })
    except Exception as e:
        pass
        
    # Palautetaan Averion data, jos yhteys onnistui ja tulevia laivoja löytyi
    if len(ships) > 0:
        return ships[:3]

    # 2. VARAJÄRJESTELMÄ (Jos pilvipalvelin on estetty tai tulevia laivoja ei löytynyt)
    for name, data in list(ship_database.items())[2:4]:
        ...
        ships.append({
            ...
            "source": "LÄHDE: TH ALGORITMI (AVERIO ESTETTIIN PILVESTÄ)"
        })
    return ships
