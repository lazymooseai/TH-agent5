...
# VR ASEMAKOODIEN SANAKIRJA
VR_STATIONS = {
    "TPE": "Tampere", "TKU": "Turku", "OLU": "Oulu", "JNS": "Joensuu", 
    "JY": "Jyväskylä", "ROV": "Rovaniemi", "KUO": "Kuopio", "PM": "Pori",
    "KOK": "Kokkola", "KEM": "Kemi", "VAA": "Vaasa", "SK": "Seinäjoki",
    "PRI": "Pori", "LPR": "Lappeenranta", "KJN": "Kajaani", "MI": "Mikkeli",
    "ILM": "Iisalmi", "RII": "Riihimäki", "HL": "Hämeenlinna", "KV": "Kouvola",
    "KTI": "Kotka", "LR": "Lappeenranta"
}
...
# --- FINTRAFFIC API (JUNAT - LÄHTÖPAIKALLA) ---
...
def fetch_live_trains(station_name):
    ...
    url = f"https://rata.digitraffic.fi/api/v1/live-trains/station/{code}?arriving_trains=50&departing_trains=0&include_nonstopping=false"
    ...
    try:
        ...
        for t in trains:
            if t.get('trainCategory') == 'Long-distance':
                ...
                # Etsitään mistä juna lähti
                origin_code = timetable[0].get('stationShortCode')
                origin_name = VR_STATIONS.get(origin_code, origin_code) # Jos nimeä ei löydy, näytetään lyhenne
...
# --- AVERIO / LAIVADATA (KORJATTU AIKAFILTTERI) ---
                ...
                if len(tds) >= 4:
                    ...
                    if clean_name in ship_database:
                        ...
                        try:
                            ...
                            # Jos laiva on "tänään" mutta yli 10 tuntia menneisyydessä, hylätään se
                            if (now - ship_dt).total_seconds() > 10 * 3600:
                                continue # Hylky, tämä on aamun laiva.
                                
                            # Jos laiva oli alle tunti sitten, se on ok.
                            if ship_dt < now - timedelta(minutes=45) and (now - ship_dt).total_seconds() > 0:
                                continue
