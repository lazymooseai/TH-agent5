...
# --- AVERIO / LAIVADATA (SCRAPING KORJATTU) ---
@st.cache_data(ttl=300)
def fetch_live_ships():
    # Tunnettujen raskaiden alusten fysiikka ja satamadata
    ship_database = {
        "MS Finlandia": {"max": 2080, "terminal": "Länsiterminaali T2 (Ei Vuosaari)"}, # Datahäiriö korjattu (Lähde: Tapahtumat 2026_2)
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # Etsitään taulukkorivit HTML-koodin sisältä
    ...
    # Palautetaan Averion data, jos yhteys onnistui
    if len(ships) > 0:
        return ships[:3]

    # 2. VARAJÄRJESTELMÄ (Jos pilvipalvelin on estetty)
