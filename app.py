...
@st.cache_data(ttl=86400) # Haetaan kerran vuorokaudessa
def get_station_names():
    try:
        res = requests.get("https://rata.digitraffic.fi/api/v1/metadata/stations", timeout=5)
        if res.status_code == 200:
            # Puhdistetaan nimistä turha " asema" -pääte pois (esim. "Helsinki asema" -> "Helsinki")
...
def fetch_live_trains(station_name):
    ...
    station_names = get_station_names() # Ladataan dynaaminen sanakirja
    ...
    try:
        ...
        for t in trains:
            if t.get('trainCategory') == 'Long-distance':
                ...
                # Etsitään mistä juna lähti ja käännetään se selkokielelle
