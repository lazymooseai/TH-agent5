    ...
    try:
        ...
        for t in trains:
            if t.get('trainCategory') == 'Long-distance':
                ...
                # SUUNTAFILTTERI: Varmistetaan, että juna on menossa kohti Helsinkiä.
                # Etsitään valitun aseman indeksi ja tarkistetaan, löytyykö sen jälkeen vielä 'HKI'
                target_idx = -1
                hki_idx = -1
                for i, row in enumerate(timetable):
                    if row.get('stationShortCode') == code and row.get('type') == 'ARRIVAL':
                        target_idx = i
                    if row.get('stationShortCode') == 'HKI':
                        hki_idx = i
                
                # Jos valittu asema ei ole Helsinki, junan pitää saapua Helsinkiin vasta valitun aseman jälkeen.
                if code != 'HKI' and (hki_idx == -1 or hki_idx < target_idx):
                    continue
                # Jos valittu asema on Helsinki, kelpaavat kaikki tänne saapuvat
                elif code == 'HKI' and target_idx == -1:
                    continue

                origin_code = timetable[0].get('stationShortCode')
                origin_name = station_names.get(origin_code, origin_code)
                
                row = timetable[target_idx]
...
def fetch_live_ships():
    ship_database = {
        [span_0](start_span)"MS Finlandia": {"max": 2080, "terminal": "Länsiterminaali T2 (Ei Vuosaari)"}, # Korjattu datahäiriö[span_0](end_span)
        "Finlandia": {"max": 2080, "terminal": "Länsiterminaali T2 (Ei Vuosaari)"}, 
...
if not live_trains:
    ...
    for train in live_trains[:limit]:
        ...
        # [span_1](start_span)Dynaaminen linkki valitun aseman mukaan[span_1](end_span)
        if st.session_state.selected_station == "PASILA":
            [span_2](start_span)url = "https://www.vr.fi/radalla?station=PSL&direction=ARRIVAL&stationFilters=%7B%22trainCategory%22%3A%22Long-distance%22%7D"[span_2](end_span)
        elif st.session_state.selected_station == "TIKKURILA":
            [span_3](start_span)url = "https://www.vr.fi/radalla?station=TKL&direction=ARRIVAL&stationFilters=%7B%22trainCategory%22%3A%22Long-distance%22%7D"[span_3](end_span)
        else:
            [span_4](start_span)url = "https://www.vr.fi/radalla?station=HKI&direction=ARRIVAL&stationFilters=%7B%22trainCategory%22%3A%22Long-distance%22%7D"[span_4](end_span)
...
events = [
    [span_5](start_span){"id": "Messukeskus", "title": "Kevätmessut", "location": "Messukeskus", "time": "17:00", "duration": "Ovet sulkeutuvat", "badge_class": "badge-info", "badge_text": "SUURI TAPAHTUMA", "border_class": "", "time_class": "", "url": "https://messukeskus.com/kavijalle/tapahtumat/tapahtumakalenteri"},[span_5](end_span)
    [span_6](start_span){"id": "Jäähalli", "title": "Jääkiekko: HIFK - Kärpät", "location": "Helsingin Jäähalli", "time": "18:30", "duration": "150 min", "badge_class": "badge-fire", "badge_text": "KORKEA KYSYNTÄ 🔥", "border_class": "red-border", "time_class": "red-text", "url": "https://liiga.fi/fi/ohjelma?kausi=2025-2026&sarja=runkosarja&joukkue=hifk&kotiVieras=koti"},[span_6](end_span)
    [span_7](start_span){"id": "Ooppera", "title": "Oopperaesitys (Tosca)", "location": "Kansallisooppera", "time": "19:00", "duration": "180 min", "badge_class": "badge-premium", "badge_text": "PREMIUM (PUKU PÄÄLLÄ)", "border_class": "yellow-border", "time_class": "yellow-text", "url": "https://oopperabaletti.fi/kalenteri/"}[span_7](end_span)
]
