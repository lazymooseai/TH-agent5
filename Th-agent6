...
def fetch_live_trains(station_name):
    ...
    # Haetaan 50 seuraavaa junaa, jotta meillä on tarpeeksi dataa suodattaa ulospäin menevät pois
    ...
    try:
        ...
        for t in trains:
            if t.get('trainCategory') == 'Long-distance':
                ...
                # SUUNTAFILTTERI: Varmistetaan, että junan absoluuttinen pääteasema on Helsinki
                final_station = timetable[-1].get('stationShortCode')
                if final_station != 'HKI':
                    continue # Hylätään maalle lähtevät junat
                ...
                for row in timetable:
                    if row.get('stationShortCode') == code and row.get('type') == 'ARRIVAL':
                        ...
                        # Aikasuodatin: Ei näytetä jo menneitä junia
                        now = datetime.now(HELSINKI_TZ)
                        if scheduled_hel < now - timedelta(minutes=10):
                            continue
        ...
        # Palautetaan täsmälleen 5 seuraavaa
        return results[:5]
    except: return []
                ...
                if len(tds) >= 4:
                    ...
                    if clean_name in ship_database:
                        ...
                        try:
                            ...
                            if (now - ship_dt).total_seconds() > 12 * 3600: ship_dt += timedelta(days=1)
                            if ship_dt < now - timedelta(minutes=45): continue
                        except: pass
